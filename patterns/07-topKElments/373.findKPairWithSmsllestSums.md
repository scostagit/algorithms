...
We sort by sum → the **3 smallest sums** are `[1,2], [1,4], [1,6]`.

---

### 🧩 But we can’t generate *all pairs* (too slow!)

If `nums1` and `nums2` each have 100,000 elements, there could be **10 billion pairs** — impossible to compute directly.

So we need a **smarter way**.

---

## ⚙️ **The Efficient Idea: Use a Min-Heap**
...
3. **Extract (pop)** the smallest pair from the heap — add it to your result.
...
→ If one array is empty, there are no pairs.

```js
let heap = [];
```

→ We'll use this as a simple min-heap.
...
→ We start with the smallest possible pairs:
...
→ Sort heap so that the smallest sum is first (in production, we’d use a real priority queue for speed).
...
→ Take the smallest pair from the heap and add it to our result.
...
→ Move to the next possible pair with the same `num1` but the next `num2`.
...
→ Decrease the counter — we found one of the desired pairs.
